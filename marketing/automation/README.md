...
Daily today post (GitHub Action `daily-today-spotlight.yml` at 08:00 UTC / ~4:00 AST). Posts **all** home **Today’s specials** when that channel has not posted yet (up to 10); after a specials post it uses happening-today only. Dedicated specials posts use a separate lock and `TODAY_SPECIALS=true`. Live publish is a loop of short Graph steps — do not fire a single POST and wait for both networks:
