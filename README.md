...
- **`POST /api/cron/meta-post`** — Publish to Facebook and/or Instagram (`dryRun`, `source: "today"` for the 13:00 UTC happening-today post, `source: "today-specials"` for a manual specials post, or `source: "weekend"`). Live today posts are stepped; use `scripts/run-today-spotlight.mjs` or the GitHub Action rather than a single curl.

GitHub Action **Daily today spotlight** (`daily-today-spotlight.yml`) posts happening-today events every day ~9:00 AST. It never posts the home **Today’s specials** rail — that is `TODAY_SPECIALS=true node scripts/run-today-spotlight.mjs`.
